...
export function YapBottomNav({ activeTab, onSelectTab }: YapBottomNavProps) {
  const navItems: { id: YapTab; label: string; isForeign?: boolean; icon: React.ElementType }[] = [
    ...
    { id: "train", label: "Latihan", icon: Dumbbell },
  ...
  return (
    <nav
      ...
      className="fixed bottom-0 left-0 right-0 z-40 flex h-16 w-full items-center justify-around border-t border-slate-800/90 bg-[#080F1E]/95 backdrop-blur-lg px-2 lg:hidden shadow-2xl select-none pb-[env(safe-area-inset-bottom,0px)]"
    >
      {navItems.map((item) => {
        ...
        return (
          <button
            ...
            className={`flex flex-1 flex-col items-center justify-center py-1.5 min-h-[48px] transition-all focus:outline-none ${
          ...
          >
            ...
            <span className={`text-[10px] mt-0.5 tracking-tight whitespace-nowrap font-medium ${item.isForeign ? "italic" : ""}`}>
              {item.label}
            </span>
          </button>
        );
