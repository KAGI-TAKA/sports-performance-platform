...
export function MfdBottomNav({ activeTab, onSelectTab }: MfdBottomNavProps) {
  ...
  return (
    <nav
      ...
      className="fixed bottom-0 left-0 right-0 z-40 flex h-20 w-full items-center justify-around border-t border-slate-200/80 dark:border-slate-800 bg-white/95 dark:bg-[#0B132B]/95 backdrop-blur-xl px-2 lg:hidden shadow-2xl select-none pb-2"
    >
      {navItems.map((item) => {
        ...
        return (
          <button
            ...
            className={`flex flex-1 flex-col items-center justify-center py-1.5 transition-all focus:outline-none ${
              isActive
                ? "text-blue-600 dark:text-amber-400 font-extrabold scale-105"
          ...
          >
            <div
              className={`p-2 rounded-2xl transition-all ${
                isActive
                  ? "bg-blue-600/15 dark:bg-amber-400/20 text-blue-600 dark:text-amber-400 shadow-sm"
                  : "bg-transparent"
              }`}
            >
